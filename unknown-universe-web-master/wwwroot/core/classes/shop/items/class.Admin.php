...
class Admin extends AbstractItem
{
    function __construct($ItemData, MySQL $MySQL)
    {
        ...
        if ($System->User->hasPremium() && $this->CURRENCY == 2 && $System->User->__get('DISCOUNT') == 0) {
            $this->PRICE = $this->PRICE * 0.8;
        } elseif ($System->User->__get('DISCOUNT') > 0 && $System->User->hasPremium() && $this->CURRENCY == 2) {
            $premium     = 0.8 - ( $System->User->__get('DISCOUNT') / 100 );
    ...
    public function buy($UserID, $PlayerID, $Amount = 1)
    {
        ...
        if (strpos($this->LOOT_ID, 'resource_') !== false) {
            ...
            if (strpos($this->LOOT_ID, 'resource_booty-key') !== false) {
                $BOOTY_KEYS = json_decode($System->User->__get('BOOTY_KEYS'), true);
                if (empty($BOOTY_KEYS)) {
                    $BOOTY_KEYS = [
                        0 => 0,
                        1 => 0,
