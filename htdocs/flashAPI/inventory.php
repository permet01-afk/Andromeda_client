...
class Connexion 
{
    ...
    public static function bdd() {     
        try 
        {
            $bdd = new PDO('mysql:host=127.0.0.1;dbname=darkorbit', 'root', 'lefaucheuristheking123');
...
function calcul_config_laser_damage($id, $config)
{
	...
	$drone_laser_dmg = $drone_laser_count[0] * 165; 

	$full_laser_dmg = $ship_laser_dmg + $drone_laser_dmg;
	$full_laser_dmg = $full_laser_dmg / 100 * 130; 
	$full_laser_dmg = $full_laser_dmg / 100 * 120; 
...
function calcul_config_shield($id, $config)
{
	...
	$drone_shield = $drone_shield_count[0] * 12000; 
