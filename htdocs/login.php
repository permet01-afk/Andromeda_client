...
function handleRegisterForm($db)
{
    ...
    } elseif (hasForbiddenControlChars($pseudo)) {
        $errors[] = 'Invalid In-Game Name (control characters are not allowed).';
    }
    ...
    $formPseudo = trim($pseudo);
    $legacyPseudo = convertToNumericEntities(htmlentities($pseudo, ENT_QUOTES, 'UTF-8'));

    $sth = $db->prepare('SELECT id FROM users WHERE username = :username OR username = :legacy_username');
    $sth->execute([
        ':username' => $formPseudo,
        ':legacy_username' => $legacyPseudo,
    ]);
    ...
    $db->insert('users_log', [
        'playerid' => $result[0]['id'],
        'message' => "<b>Welcome on <font color='#0080FF'>Andromeda</font></b> (beta)<br/>Your firm gave you 200.000 U.<br/>Have fun !<br/>",
    ]);
