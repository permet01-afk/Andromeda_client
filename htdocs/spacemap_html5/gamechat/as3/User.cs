...
namespace DarkorbitChat.Game
{
    public class User
    {
        ...
        private static bool TryExtractSyntheticGroupChat(string rawMessage, out string groupKey, out string body)
        {
            ...
            var message = rawMessage.TrimStart();
            const string prefix = "[[GRPCHAT:";
            if (!message.StartsWith(prefix, StringComparison.Ordinal))
            {
                return false;
            }

            var suffixIndex = message.IndexOf("]]", StringComparison.Ordinal);
            if (suffixIndex <= prefix.Length)
            {
                return false;
            }

            groupKey = message.Substring(prefix.Length, suffixIndex - prefix.Length).Trim();
            body = message.Substring(suffixIndex + 2).TrimStart();
            return groupKey.Length > 0;
        }

        private string ComposeSyntheticGroupChatPacket(string groupKey, string senderName, string messageBody)
        {
            if (String.IsNullOrEmpty(Clan))
            {
                return "gx%" + groupKey + "@" + senderName + "@" + messageBody + "#";
            }

            return "gx%" + groupKey + "@" + senderName + "@" + messageBody + "@" + Clan + "#";
        }
                    ...
                    else if (cmd == "/w")
                    {
                        foreach (var pair in Program.Users)
                        {
                            ...
                            if (String.Equals(pair.Value.Username.ToLower(), user.ToLower(), StringComparison.CurrentCultureIgnoreCase) && pair.Value.ChatsJoined.Contains(Convert.ToInt32(roomId)))
                            {
                                ...
                                string groupKey;
                                string groupBody;
                                if (TryExtractSyntheticGroupChat(message, out groupKey, out groupBody))
                                {
                                    pair.Value.Send(pair.Value.ComposeSyntheticGroupChatPacket(groupKey, Username, groupBody));
                                    Program.AddLog(Username, user, "Group: " + groupBody);
                                    return;
                                }
